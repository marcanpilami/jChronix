...
public class TestEnvVarAndParams extends TestBase
{
    ...
    public void testEnvVar() throws JMSException
    {
        ...
        RunnerCommand sc1 = PlanBuilder.buildShellCommand(a, "echo %myvar1%", "case 1", "");
        State s1 = PlanBuilder.buildState(c, pg1, sc1);
        s1.addEnvVar("myvar1", "houba");
        c.getStartState().connectTo(s1);

        RunnerCommand sc2 = PlanBuilder.buildShellCommand(a, "echo %myvar2%", "case 2", "");
        sc2.addEnvVar("myvar2", "pouet");
        State s2 = PlanBuilder.buildState(c, pg1, sc2);
        s1.connectTo(s2);

        RunnerCommand sc3 = PlanBuilder.buildShellCommand(a, "echo %myvar3%", "case 3", "");
        sc3.addInternalEnvVar("myvar3", "pouet");
        State s3 = PlanBuilder.buildState(c, pg1, sc3);
        s2.connectTo(s3);

        RunnerCommand sc4 = PlanBuilder.buildShellCommand(a, "echo %myvar4%", "case 4", "");
        sc4.addEnvVar("myvar4", "pouet");
        State s4 = PlanBuilder.buildState(c, pg1, sc4);
        s4.addEnvVar("myvar4", "hop");
        s3.connectTo(s4);

        RunnerCommand sc5_1 = PlanBuilder.buildShellCommand(a, "echo set SUPERVAR=123", "case 5 1of2", "");
        State s5_1 = PlanBuilder.buildState(c, pg1, sc5_1);
        s4.connectTo(s5_1);
        RunnerCommand sc5_2 = PlanBuilder.buildShellCommand(a, "echo %SUPERVAR%", "case 5 2of2", "");
        State s5_2 = PlanBuilder.buildState(c, pg1, sc5_2);
        s5_1.connectTo(s5_2);

        RunnerCommand sc6 = PlanBuilder.buildShellCommand(a, "echo %myvar1%", "case 6", "");
        State s6 = PlanBuilder.buildState(c, pg1, sc6);
        s5_2.connectTo(s6);

        RunnerCommand sc7 = PlanBuilder.buildShellCommand(a, "echo %myvar7%", "case 7", "");
